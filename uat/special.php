<?
    ...
    $where_clause="1 and is_hide = '2' order by rank desc";# and is_hide = '2'
    $tbl_name="sys_portal_l1";
    getall_data($tbl_name, $where_clause, $all_product_menu);
	$l1_id = $_GET['l1_id'];
	if(!$l1_id)	$l1_id = $all_product_menu[0]['Fmain_id'];
    ...
    if($l1_id){
		...
		$x100_cnt_info=array();
		$query_string = "SELECT t.*, td.rank AS new_rank FROM `sys_portal_x100_cnt` AS t INNER JOIN ".$MYSQL_TABS['portal_l1_cnt_rank']." AS td ON t.Fmain_id = td.portal_l1_cnt_id WHERE td.portal_l1_id = '".$l1_id."' ".$add_sql." order by new_rank desc";
                                ...
                                <ul class="item_menu_list slides">
                                    <?
			                            ...
			                            for($iii=0;$iii<count($all_product_menu);$iii++){
			                                ...
			                                else if($l1_id==$all_product_menu[$iii]['Fmain_id']){
			                                    $active = "active";
			                                }
			                                if($l1_id){
				                                $the_id = "l1_id";
		                            ...
		                            ?>
		                                <li class="<?=$active?>">
		                                    <a href="special.php?<?=$the_id?>=<?=$all_product_menu[$iii]['Fmain_id']?>"><?=$show_str?>
		                                        <div class="f16">
...
<!--                             <a href="<?=$info[$iii]['text_field_1']?>.html"> -->
                            <a href="special-detail.php?num=<?=$info[$iii]['text_field_1']?>&l1_id=<?=$l1_id?>">
