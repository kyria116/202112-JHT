...
//exit;
    ...
    if($order_info['pay_state']!= "出貨中" && $order_info['pay_state'] != "已出貨未付款"){
	    print "訂單狀態無法開發票！";
    	exit;
    }
...
//
 ...
 if($sale_product_money > 0)
 {
    for($i=0;$i<count($order_cnt_info);$i++)
    {
	    if((int)$order_cnt_info[$i]['small_price'] >= (int)$sale_product_money && $sale_product_money > 0)
		{
			if($order_cnt_info[$i]['amount'] > 1 && $order_cnt_info[$i]['price']>$sale_product_money){#如果買大於1個，且單價比折扣高，就拆成兩筆
				$order_cnt_info[$i]['amount'] = $order_cnt_info[$i]['amount'] - 1;
				$order_cnt_info[$i]['small_price'] = $order_cnt_info[$i]['amount'] * $order_cnt_info[$i]['price'];
				
				$new_money = (int)$order_cnt_info[$i]['price'] - (int)$sale_product_money;
				$new_info = array();
				$new_info = $order_cnt_info[$i];
				$new_info['price'] = $new_money;
				$new_info['small_price'] = $new_money;
				$new_info['amount'] = 1;
				$order_cnt_info[] = $new_info;
				$sale_product_money=0;
			}
			else{
